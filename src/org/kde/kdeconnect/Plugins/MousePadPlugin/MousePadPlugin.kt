...
class MousePadPlugin : Plugin() {
    ...
    override fun getUiButtons(): List<PluginUiButton> = listOf(
        PluginUiButton(context.getString(R.string.open_mousepad), R.drawable.touchpad_plugin_action_24dp) { parentActivity ->
            val intent = Intent(parentActivity, MousePadActivity::class.java)
            intent.putExtra("deviceId", device.deviceId)
            parentActivity.startActivity(intent)
        }
    ) + if (device.deviceType == DeviceType.TV) listOf(
            PluginUiButton(context.getString(R.string.open_mousepad_tv), R.drawable.tv_remote_24px) { parentActivity ->
